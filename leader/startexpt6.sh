...
for i in $(seq 2 $((n + 1))); do
  if [ $i -eq 2 ] || [ $i -eq 6 ]; then
    ssh -o StrictHostKeyChecking=no node-$i "\
    cd $prefix; cgexec -g memory:janus taskset -ac 1 bin/server -maddr=${masterAddr} -mport=${masterPort} -addr=node-$i -port=${serverPort} -e=$doEpaxos -copilot=$doCopilot -latentcopilot=$doLatentCopilot -exec=$exec -dreply=$reply -durable=$durable -p=$cpus -thrifty=$thrifty" \
